          ...
          <div className="lg:p-4 space-y-6 md:flex md:gap-20 justify-center md:space-y-0 lg:items-center">
            ...
            <div className="md:w-7/12 lg:w-1/2">
              <h2 className="text-3xl font-bold text-zinc-900 md:text-4xl dark:text-white">
                {translations["en-US"].about.rawJs}
              </h2>
              <p className="text-lg my-8 text-zinc-600 dark:text-zinc-300">
                {translations["en-US"].about.rawJsDescription}{" "}
                <a
                ...
                >
                  {translations["en-US"].about.jsBenchmark}
                </a>
                .
              </p>
              <div className="divide-y space-y-4 divide-zinc-100 dark:divide-zinc-800">
                <div className="mt-8 flex gap-4 md:items-center">
                  ...
                  <div className="w-5/6">
                    <h4 className="font-semibold text-lg text-zinc-700 dark:text-purple-300">
                      {translations["en-US"].about.seventyPercent}
                    </h4>
                    <p className="text-zinc-500 text-sm dark:text-zinc-400">
                      {translations["en-US"].about.benchmarkWarning}
                    </p>
                  </div>
                </div>
                <div className="pt-4 flex gap-4 md:items-center">
                  ...
                  <div className="w-5/6">
                    <h4 className="font-semibold text-lg text-zinc-700 dark:text-purple-300">
                      {translations["en-US"].about.integrate}
                    </h4>
                    <p className="text-zinc-500 text-sm dark:text-zinc-400">
                      {translations["en-US"].about.noNeedLearn}
                    </p>
                  </div>
                </div>
              </div>
              <p className="mt-10 text-xs bg-gradient-to-b dark:from-zinc-500 dark:to-[#111] dark:hover:to-zinc-500 inline-block text-transparent bg-clip-text from-zinc-500 to-white hover:to-zinc-500 opacity-40 hover:opacity-50 transition-opacity">
                {translations["en-US"].about.benchmarkNote}
              </p>
            </div>
          </div>
        </Container>
      ...
      <div className="relative">
        <Container>
          <h3 className="text-2xl text-center font-bold text-zinc-900 dark:text-white md:text-3xl lg:text-4xl">
            {translations["en-US"].about.whatsInMillionJS}
          </h3>
          <p className="mt-3 text-center text-zinc-600 dark:text-zinc-300 md:text-md lg:text-lg">
            {translations["en-US"].about.toolsToMakeReactFaster}
          </p>
          <div className="relative mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            <Card
              title={translations["en-US"].about.blockVirtualDom}
              icon={<BoxIcon />}
              description={
                <>
                  {translations["en-US"].about.millionIntroduces}{" "}
                  <Link
                    href="/blog/virtual-dom"
                    className="underline nx-text-primary-600"
                  >
                    {translations["en-US"].about.blockVirtualDomQuote}
                  </Link>{" "}
                  {translations["en-US"].about.blockVirtualDomDescription}
                </>
              }
            />
            <Card
              title={translations["en-US"].about.superchargedCompiler}
              icon={<LightBulbIcon />}
              description={
                <>
                  {translations["en-US"].about.millionUses}{" "}
                  <Link
                    href="/blog/behind-the-block"
                    className="underline nx-text-primary-600"
                  >
                    {translations["en-US"].about.customCompiler}
                  </Link>{" "}
                  {translations["en-US"].about.automaticallyOptimizes}
                </>
              }
            />
            <Card
              title={translations["en-US"].about.automaticMode}
              icon={<ThumbsUpIcon />}
              description={
                <>
                  {translations["en-US"].about.tiredOf}{" "}
                  <Link
                    href="/docs/automatic"
                    className="underline nx-text-primary-600"
                  >
                    {translations["en-US"].about.dropIn}
                  </Link>{" "}
                  {translations["en-US"].about.makeReactFaster}
                </>
              }
            />
          </div>
      ...
      {!showShowdown ? (
        <div className="bg-white p-4 pb-6 dark:bg-zinc-900 rounded-lg">
          <div className="w-full">
            <p className="font-bold text-lg">{translations["en-US"].about.jsBenchmark}</p>
            <p className="text-md mt-1 text-zinc-700 dark:text-zinc-400">
              {translations["en-US"].about.higherBetter}
            </p>
            <Chart />
          </div>
          <div className="text-sm text-zinc-400">
            {translations["en-US"].about.basedOn}{" "}
            <a
              href="https://krausest.github.io/js-framework-benchmark/2023/table_chrome_112.0.5615.49.html"
              target="_blank"
              rel="noreferrer"
            ...
            >
              {translations["en-US"].about.benchmarkData}
            </a>{" "}
            {translations["en-US"].about.chromeVersion}
          </div>
        </div>
      ) : (
        <></>
...
function GraphicWrapper({ children, onClick }: { children: JSX.Element; onClick: () => void }) {
  return (
    <Tilt
      tiltMaxAngleX={5}
      tiltMaxAngleY={10}
  ...
  return (
    ...
    >
      <div className="relative flex h-full flex-col gap-6 rounded-2xl bg-zinc-100 p-8 dark:bg-zinc-900">
        {icon}
        <div>
          <h4 className="text-xl font-semibold text-zinc-900 dark:text-white">{title}</h4>
